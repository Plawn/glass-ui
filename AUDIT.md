...
Beaucoup de composants hardcodent `"transition-all duration-300 ease-out"` etc. au lieu
d'importer depuis `src/constants/animations.ts` (contraire à la convention CLAUDE.md).
- **Trouver** : `grep -rn "duration-\|ease-" src/components --include="*.tsx"`
- Ajouter des constantes de transition manquantes (`TRANSITION_COLORS_FAST`, etc.)
...
### 4. Constantes d'animation non adoptées
Beaucoup de composants hardcodent `"transition-all duration-300 ease-out"` au lieu
d'importer depuis `src/constants/animations.ts`.
- `grep -rn "duration-\|ease-" src/components --include="*.tsx"`
- Ajouter les constantes manquantes (`TRANSITION_COLORS_FAST`...) puis remplacer.
